...
You can toggle the global option `downsize` with calls to `scale_up()` and `scale_down()`, and you can override it with `downsize(..., downsize = L)`, where `L` is `TRUE` or `FALSE`. You can also check the `downsize` option with the `scaling()` function, as in these examples.
